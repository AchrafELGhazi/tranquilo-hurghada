...
export const BookingDetailsModal: React.FC<BookingDetailsModalProps> = ({
    ...
    const actions = (
        <>
            <button
                onClick={onClose}
                className='px-6 py-2 text-[#C75D2C] hover:bg-[#F8B259]/20 rounded-lg transition-colors font-medium cursor-pointer'
            >
                Close
            </button>
                ...
                <Section title='Stay Details' icon={<Calendar className='w-4 h-4 text-[#D96F32]' />}>
                    <div className='bg-white/30 rounded-lg p-4 border border-[#F8B259]/20'>
                        <div className='grid grid-cols-2 gap-4 mb-4'>
                            <InfoGridItem label='Check-in' value={`${checkInDate.formatted} (${checkInDate.day})`} />
                            <InfoGridItem label='Check-out' value={`${checkOutDate.formatted} (${checkOutDate.day})`} />
                            <InfoGridItem
                                label='Duration'
                ...
                <Section title='Payment Information' icon={<CreditCard className='w-4 h-4 text-[#D96F32]' />}>
                    <div className='bg-white/30 rounded-lg p-4 border border-[#F8B259]/20 space-y-3'>
                        ...
                        <InfoRow
                            ...
                            value={booking.paymentMethod === 'BANK_TRANSFER' ? 'Bank Transfer' : 'Payment on Arrival'}
                        />
                        <InfoRow
